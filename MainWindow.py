...
class Game:
    def __init__(self):
        ...
        self.game_over_delay = 4000  # 4 seconds in milliseconds
    ...
    def run(self, screen):
        if self.game_over:
            ...
            if current_time - self.death_time > self.game_over_delay:
                pygame.quit()
                sys.exit()
            else:
                screen.fill((0, 0, 0))
            ...
            self.player.update()
            if self.player.sprite.toggle_pause():
                screen.fill((0, 0, 0))
                pause_surface = self.font.render(f'PAUSED', False, 'white')
                pause_rect = pause_surface.get_rect(topleft=((screen_width / 2) -55, (screen_height / 2) - 60)) # (screen_width / 2), (screen_height / 2)
...
if __name__ == '__main__':
    ...
    pygame.time.set_timer(ALIENLASER, 2000)  # milliseconds 2000
    ...
    while True:
        ...
        game.run(screen)  # pass the screen to the run method
        crt.draw()
